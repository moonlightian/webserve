...
class http_conn//主线程创建的http类
{
public:
    static const int FILENAME_LEN = 200;
    static const int READ_BUFFER_SIZE = 2048; //设置读写缓冲区大小
...
public:
    void init(int sockfd, const sockaddr_in &addr);//初始化套接字（从这个socket上读取写入）地址，函数内部会调用私有方法init
...
private:
    void init();
    HTTP_CODE process_read(); //从m_read_buf读取，并处理请求报文，返回状态码
